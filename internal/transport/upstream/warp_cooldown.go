...
// slot of the named upstream and reports how many slots were serving a live
// cooldown. It is the counterpart of the 429-driven WARP rotation in attempt.go:
// the 429 was charged to the key slot, but the quota it reflects belongs to the
// egress IP that the rotation just replaced, so keeping the penalty would idle a
// healthy identity for up to MaxCooldownDuration (measured: 302s of a 300s cap in
// the e2e sandbox).
//
...
func ClearUpstreamKeyCooldowns(provider SnapshotProvider, upstreamName string, logger *slog.Logger) int {
	...
	cleared := u.KeyRing.ClearCooldowns(time.Now().UnixNano())
