...
func (s *Server) buildHandler(deps RouterDeps) http.Handler {
	...
	// endpoints themselves stay gated).
	...
	// Provider & Credential Pool Administration API (admin-authorized). Providers
	// and keys live only in Turso, so these handlers fail closed without a
	// configured store.
