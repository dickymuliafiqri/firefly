...
// clients. Each begins as the embedded bytes and is swapped by
// SetBuildVersion for a copy whose info.version matches the running build.
...
// SetBuildVersion stamps the running build's version into both served OpenAPI
// documents so info.version cannot drift from the release that ships them. The
