...
> **Status:** DISETUJUI & terimplementasi untuk Fase 0–2c (commit `81b52c9`…`c14c51a`, rilis **v1.13.0**), plus K3-c (permukaan baca legacy hint-only + dashboard *Bind Provider Keys*), plus kode Fase 3 di repo harvester (belum di-deploy). **Perubahan 2026-09-22: seluruh permukaan mesin DIHAPUS** — `POST /api/harvester/sync`, token service, dan spec-nya tidak ada lagi di kode; harvester menulis lewat API admin dengan token admin (lihat §3.3, §11.3, dan Riwayat Revisi). Fase 4 **belum dimulai** — gerbangnya keputusan O10, O13, dan lama masa observasi (**O14 sudah terjawab** oleh penghapusan ini).
...
| Sinyal reload katalog | `catalog_revisions` + `GetKeysState` (`MAX(updated_at)`, `COUNT(*)`) dipantau syncer — **base 60 detik dengan backoff idle sampai 5 menit** (sejak 2026-09-22; sebelumnya tick tetap 15 detik), dan tiap tulis lokal memicu `TriggerSync` | `syncer.go:20-22` (`DefaultSyncInterval`/`DefaultSyncMaxInterval`), `syncer.go:95-104`, `syncer.go:123-139`; flag `-turso-sync-interval`/`-turso-sync-max-interval` |
...
### 3.3 Keputusan 2026-09-22 — permukaan mesin dihapus

**D5.** `POST /api/harvester/sync`, `service_auth.go` (`authorizeService` + `RouterDeps.ServiceToken`), `openapi-harvester.yaml` + drift test-nya, dan flag `-harvester-token`/`FIREFLY_HARVESTER_TOKEN` **dihapus dari kode**. Harvester kini memakai **API admin** dengan **token admin** (`-admin-token` / `FIREFLY_ADMIN_TOKEN`):

| Kebutuhan harvester | Permukaan pengganti |
|---|---|
| batch upsert provider + key (termasuk `status`, `expires_at`, `account_metadata`) | `POST /api/providers/{id}/keys` → `UpsertProviderKeyRecords` (jalur store yang sama; satu provider per panggilan) |
| patch status / rotasi secret dengan `id` baris stabil | `PATCH /api/keys/{id}` (O11) |
| baca balik barisnya sendiri | `GET /api/providers/{id}/keys` (hint-only) — **O14** terjawab tanpa endpoint baru |

**Alasan:** kedua permukaan sudah menulis lewat jalur store yang sama (`ProviderKeySyncEntry` → `upsertKeyTx`). Yang tersisa dari endpoint mesin hanyalah identitas auth kedua, satu panggilan per batch lintas provider, dan deaktivasi massal — sementara biayanya dua kontrak yang harus dijaga sinkron, dua spec, dan satu rahasia otomasi tambahan. Satu permukaan = satu aturan validasi, satu guard, satu spec.

**Harga yang diketahui & disetujui saat keputusan diambil:**
- **Deaktivasi massal hilang.** `deactivate_keys` tidak ada lagi; N key = N panggilan `PATCH /api/keys/{id}`. Sifatnya sama (hanya id yang disebut yang berubah), hanya granular. Harvester sudah menyimpan id-nya di outbox.
- **Satu panggilan = satu provider**, bukan satu snapshot lintas provider. Idempotensinya tetap: natural key `api_key` (UNIQUE global), `id` baris stabil, replay = `unchanged`.
- **Harvester memegang token admin**, dan token itu membuka seluruh permukaan admin (tenant, settings, log, telemetry), bukan hanya provider/key. Guard-nya `authorizeAdmin`: token admin diterima lewat *static-token bypass* di `auth.Manager.ValidateToken`, dan bila **tidak ada** token admin maupun vault auth, `authorizeAdmin` mengizinkan (perilaku lama untuk unit test) — jadi permukaan admin sekuat konfigurasi auth deployment-nya. Rotasi token = restart node.
- **Satu mutasi = satu bump revisi katalog**, bukan satu per snapshot lintas provider. Untuk batch lintas provider ini lebih ringan; deaktivasi massal menjadi N reload, yang tetap benar karena tiap reload deterministik (§8).

**Konsekuensi yang baru tertelusuri (2026-09-22, setelah keputusan):** nilai `status` di luar allowlist operator jadi tak-tertuliskan. Dua permukaan yang tersisa sama-sama menegakkan `keyStatuses` (`active`/`deactivated`/`expired`/`revoked`), sedangkan jalur sync dulu pass-through untuk nilai berbentuk baik apa pun — jadi `status="dead"` (dipakai `scripts/backfill_grok_cli_tokens.py`, Lampiran B W5) kini ditolak 400 di API. Padanan semantiknya `revoked`/`deactivated`; bila nilai literal `dead` memang harus dipertahankan, jalurnya runbook DB sekali-pakai yang sudah disediakan Lampiran B.2.

**Test yang selamat:** matriks permukaan mesin dipindah ke permukaan admin — `providers_admin_test.go` (HTTP: batch, reassign, expiry, rotasi, no-leak) dan `provider_store_test.go` (store: create/upsert/patch, idempoten, metering utuh, vault, status→routability, intent expiry, mirror credential). Test **auth service-token (401/503) tidak punya padanan**: guard-nya kini `authorizeAdmin`, yang sudah diuji test admin yang ada.
...
> **Koreksi 2026-09-22:** draf §5 ini hanya mencantumkan file yang *sengaja* akan disentuh. Enam file lain ternyata ikut berubah dan baru dicatat setelah implementasi: `settings.go`, `store.go`, `store_test.go`, `errors.go`, `syncer.go`, plus `schema_test.go` (test Fase 1 yang belum terdaftar di draf) — semuanya cocok dengan `git status` hari ini. `syncer.go` khususnya bukan kosmetik: tanpa mutex-nya, `TriggerSync` dari jalur tulis admin bisa tumpang tindih dengan tick periodik dan menjalankan dua siklus reload di atas cursor yang sama.

> **Koreksi 2026-09-22 (kedua):** lima berkas BARU di blok atas **dihapus** bersama permukaan mesin — `service_auth.go`, `harvester_api.go`, `harvester_api_test.go`, `openapi/openapi-harvester.yaml`, `openapi_harvester_test.go` — mengikuti §3.3. `provider_store.go` kehilangan mesin `SyncProviderKeys` (sisanya `UpsertProviderKeyRecords` + helper `upsertKeyTx`/`activeKeyIDsTx` yang sama-sama dipakai, plus `PatchProviderKeyRecord`/`DeleteProviderKeyRecord`), dan `models.go` kehilangan DTO request/result sync yang jadi tak terpakai. Harness test-nya pindah ke `providers_admin_test.go` dan `provider_store_test.go`.
...
### Fase 2a — ~~Endpoint Mesin + Auth Service~~ **DIHAPUS 2026-09-22**

> **Dihapus 2026-09-22 (§3.3).** Endpoint, token service, spec, dan seluruh langkah 1–7 di bawah tidak lagi ada di kode. Blok ini dipertahankan sebagai catatan sejarah kontrak yang pernah disepakati; padanannya kini `POST /api/providers/{id}/keys` + `PATCH /api/keys/{id}`.

**Tujuan (historis):** `POST /api/harvester/sync` hidup, idempoten, dan aman.
...
> **Dikoreksi 2026-09-22 (§3.3):** blok `FIREFLY_HARVESTER_TOKEN` di atas tidak lagi berlaku — variabel itu sudah dihapus dari Firefly. Yang berlaku sekarang: node Firefly cukup punya **token admin** (`-admin-token` / `FIREFLY_ADMIN_TOKEN`, sama dengan yang dipakai dashboard), dan sisi harvester diarahkan ke endpoint admin (`/api/providers*`, `/api/keys/{id}`) alih-alih `/api/harvester/sync`. Karena kontraknya berubah, klien Fase 3 di repo harvester (`core/firefly_api.py` yang menembak `/api/harvester/sync`) **wajib diperbarui sebelum deploy** — dan itu pekerjaan di repo harvester, bukan di sini. Mode `api`/`db`/`auto` tetap ada sebagai flag rollback, tetapi mode `api` kini menembak permukaan admin.
...
3. Hapus token read-only — **hanya** setelah jalur baca pengganti siap. **O14 ditutup 2026-09-22 (§3.3):** penggantinya `GET /api/providers/{id}/keys` dengan token admin, jadi token read-only boleh dicabut begitu harvester memakai jalur admin.
...
**Kesehatan test — diputuskan dibiarkan (K14, 2026-09-22):** dua flake report-only, keduanya timing/lingkungan dan bukan bug fungsional — (1) penyegar OAuth dan (2) `TestKeyRing_ThroughputExceeds10MillionOps` bila dijalankan di bawah `./...` (bersaing CPU dengan paket lain; lolos saat paketnya dijalankan sendiri). Keputusan: **biarkan**, tidak diinvestigasi pada fase ini; suite tetap hijau pada run normal dan tak satu pun menyentuh jalur migrasi. Dicatat sebagai risiko yang diketahui, bukan blocker commit. **K14 dikoreksi sebagian (2026-09-23):** flake (1) ternyata **bukan** flake lingkungan — `Store.List` mengembalikan koneksi dalam urutan acak (iterasi map), sementara `TestRefresher_Tick_ContextCancellation` mengasumsikan `conn-1` selalu di-refresh lebih dulu; ketika `conn-2` yang dilayani lebih dulu, ctx dibatalkan di delay antar-akun dan `conn-1` tak pernah disentuh → `require.Eventually` timeout. Direproduksi tanpa kontensi CPU (hanya test ini yang dipilih): 4/20 run lalu 1/40 run gagal, dan setiap kegagalan berlog `connection_id=conn-2`. Ini penyebab CI merah pada `73b19dd`. Diperbaiki di commit ini secara test-only: mock melaporkan `conn.ID` yang dilayani dan assertion mengikuti ID itu, bukan `conn-1` hardcoded; kode produksi tidak berubah. Flake (2) `TestKeyRing_ThroughputExceeds10MillionOps` tetap dinilai lingkungan dan tetap terbuka.
...
| Node lain tidak konvergen | Key baru tidak terpakai di node lain | `pushed` di respons; `TriggerSync` di node penulis; pull periodik di node lain (base 60 detik, backoff ke 5 menit saat idle — sejak 2026-09-22); uji 2 node |
...
- [x] ~~`POST /api/harvester/sync` idempoten~~ **digantikan 2026-09-22 (§3.3)**: `POST /api/providers/{id}/keys` idempoten — retry tidak mencetak baris, `id` tidak berubah, respons tanpa rahasia. *(`provider_store_test.go`, `providers_admin_test.go`)*
- [x] ~~Auth service fail-closed, terpisah dari `authorizeAdmin`, teruji (401/503)~~ **dihapus 2026-09-22 (§3.3)**: harvester memakai `authorizeAdmin` dengan token admin; test 401/503 service-token tidak punya padanan, sedangkan 413 body tetap berlaku lewat `readAdminJSON`.
...
| 2a — endpoint mesin | ~~Selesai · `8a6f792`~~ **Dihapus 2026-09-22** | Endpoint + `service_auth.go` + spec dihapus (§3.3); test-nya pindah ke `providers_admin_test.go`/`provider_store_test.go`; `syncer_test.go` tetap |
...
### 11.1 Sudah diputuskan & terimplementasi (O1–O9 disetujui 2026-09-21; O11–O12 2026-09-22; **O1/O2/O4/O5 digantikan 2026-09-22 — lihat §11.3**)

| # | Keputusan | Wujud di kode |
|---|---|---|
| O1 | ~~snapshot tunggal, bukan granular~~ **digantikan 2026-09-22** | Batch **per provider**: `POST /api/providers/{id}/keys` → `UpsertProviderKeyRecords` — satu batch = satu transaksi = satu bump revisi |
...
| O4 | ~~token service di env var, fail-closed~~ **dihapus 2026-09-22** | `FIREFLY_HARVESTER_TOKEN`, `RouterDeps.ServiceToken`, dan `authorizeService` tidak ada lagi; harvester memakai token admin (`-admin-token`) lewat `authorizeAdmin` |
| O5 | ~~prefix `/api/harvester/*`~~ **dihapus 2026-09-22** | Rute mesin hilang; harvester memakai `/api/providers*` + `/api/keys/{id}` — prefix yang sama dengan dashboard |
...
| O8 | `account_metadata` tidak pernah diekspos | write-only di permukaan tulis yang tersisa, diafirmasi `TestProvidersAdmin_NeverLeaksSecretsOrMetadata` (tiga respons: list key, get provider, delete key) + `TestUpsertProviderKeyRecords_AccountMetadataVaultSemantics` di level store. **Dikoreksi 2026-09-22:** jalur sync dihapus (§3.3), jadi `harvester_api_test.go` yang dulu ikut memikul afirmasi ini hilang bersamanya |
...
| O14 | ~~kapabilitas baca pengganti belum ada di jalur API~~ **ditutup 2026-09-22 (§3.3)** | Diputuskan **tidak** menambah endpoint baca service-token: harvester memakai `GET /api/providers/{id}/keys` (admin, hint-only) dengan token admin — permukaan yang sama dengan dashboard. Konsekuensinya tercatat di §3.3: token admin menjadi kredensial otomasi harvester, jadi hak bacanya seluas permukaan admin, bukan hanya barisnya sendiri |

### 11.3 Ditutup/digantikan setelah implementasi (2026-09-22)

| # | Keputusan | Wujud di kode |
|---|---|---|
| **D5** | permukaan mesin (`POST /api/harvester/sync`) + token service + spec-nya **dihapus**; harvester menulis lewat API admin dengan token admin | endpoint, `service_auth.go`, `openapi-harvester.yaml`, `openapi_harvester_test.go`, `harvester_api.go`+`_test.go`, `SyncProviderKeys`, dan flag `-harvester-token` hilang; padanannya `UpsertProviderKeyRecords`/`PatchProviderKeyRecord` |
| O1 | ~~snapshot tunggal~~ | batch per provider (`POST /api/providers/{id}/keys`) |
| O2 | ~~deaktivasi hanya lewat id eksplisit~~ **permukaan batch-nya hilang** | tidak ada batch deaktivasi; N key = N `PATCH /api/keys/{id}`, sifatnya tetap eksplisit (hanya id yang disebut yang berubah) |
| O4/O5 | ~~token service + prefix `/api/harvester/*`~~ | guard `authorizeAdmin` + prefix `/api/providers*` |
| O14 | ~~endpoint baca service-token~~ | `GET /api/providers/{id}/keys` (admin, hint-only) |
| I7 | "harvester hanya upsert + deactivate" | kini **kebijakan, bukan pembatas teknis**: harvester memegang token admin, jadi hard-delete yang dulu tertutup baginya hanya dijaga disiplin sisi harvester (§3.3) |
...
> **Catatan 2026-09-22 (setelah D5, §3.3): lampiran ini snapshot pra-D5.** Inventaris penulis dan
> `file:line`-nya tidak berubah, tetapi kolom *"Ekspresibel via `POST /api/harvester/sync`?"* kini
> dibaca terhadap permukaan pengganti — `POST /api/providers/{id}/keys` + `PATCH /api/keys/{id}` —
> karena endpoint mesin sudah tidak ada. Satu verdict berubah karena penghapusan itu: **pass-through
> status asing hilang.** Dua permukaan yang tersisa menegakkan allowlist `keyStatuses`, jadi W5
> (mark-dead `status="dead"`) tidak lagi ekspresibel lewat API mana pun — padanannya
> `revoked`/`deactivated`, atau runbook DB sekali-pakai bila nilai literalnya harus dipertahankan (B.2).
...
| 2026-09-22 | **Permukaan mesin dihapus (D5): harvester pindah ke API admin (§3.3, §11.3)** — atas keputusan pemilik proyek, `POST /api/harvester/sync`, `service_auth.go` (`authorizeService` + `RouterDeps.ServiceToken`), spec `openapi-harvester.yaml` + drift test-nya, flag `-harvester-token`/`FIREFLY_HARVESTER_TOKEN`, dan mesin store `SyncProviderKeys` **dihapus dari kode**. Harvester kini memakai permukaan operator dengan token admin: `POST /api/providers/{id}/keys` (jalur store yang sama, intent `expires_at` tiga-nilai, `account_metadata` write-only), `PATCH /api/keys/{id}` untuk status/rotasi, dan `GET /api/providers/{id}/keys` (hint) sebagai jawaban **O14** — tidak ada endpoint baca service-token baru. Harga yang disetujui: `deactivate_keys` hilang (N key = N `PATCH`, sifat eksplisitnya sama), batch per provider, dan token admin menjadi kredensial otomasi harvester sehingga **I7 menjadi kebijakan, bukan pembatas teknis** (§3.3). Test store dimigrasikan ke entry point admin (`provider_store_test.go` ditulis ulang: create/upsert/patch, idempoten + metering utuh, vault, status→routability, intent expiry, reassign, validasi batch) dan harness HTTP pindah ke `providers_admin_test.go`; ditemukan & diperbaiki satu cacat kontrak store: **batch yang gagal melaporkan counter yang sudah di-rollback** (`UpsertProviderKeyRecords` kini mengembalikan hasil kosong saat error). Prose/kode yang menyebut permukaan mesin disinkronkan (`syncer.go`, `store.go`, `models.go`, `openapi-admin.yaml`, `lib/secret.ts`), dokumen pembaca (`README.md`, `AGENTS.md`) kehilangan rute + flag-nya, dan `CHANGELOG.md` mendapat bagian `[Unreleased]`/`Removed`/`Fixed`. Koreksi lama yang menyertai: klaim "tick sync 15 detik" di §2/§8 diganti pacing sebenarnya (base 60 detik, backoff 5 menit, flag `-turso-sync-interval`/`-turso-sync-max-interval`) |
| 2026-09-22 | **Konsekuensi D5 ditelusuri ke dokumen analisis** — narrowing kapabilitas yang tidak masuk daftar harga saat keputusan diambil kini tercatat: §3.3 dapat paragraf *konsekuensi baru tertelusuri* (**`status="dead"` tak lagi ekspresibel** — allowlist `keyStatuses` berlaku di dua permukaan tersisa, padanannya `revoked`/`deactivated` atau runbook DB sekali-pakai), baris O8 di §11.1 diberi afirmasi pengganti (`TestProvidersAdmin_NeverLeaksSecretsOrMetadata` + `TestUpsertProviderKeyRecords_AccountMetadataVaultSemantics`, karena `harvester_api_test.go` ikut terhapus), Lampiran B diberi catatan **snapshot pra-D5** di kepalanya (kolom ekspresibilitas dibaca terhadap permukaan admin; verdict W5 berubah), dan ekor koreksi B.3.1/B.3.2 diberi klausa D5 (`SyncProviderKeys` hilang, tinggal dua permukaan operator yang tetap berbagi `expiryIntent()`) |
...
| 2026-09-23 | **K14 dikoreksi sebagian** — flake penyegar OAuth bukan flake timing/lingkungan: `Store.List` mengembalikan koneksi dalam urutan acak (map), test meng-hardcode `conn-1` sehingga ~8% run gagal (terukur 4/20 dan 1/40, tanpa beban paralel) dan itu penyebab CI merah pada `73b19dd`. Diperbaiki di commit ini (test-only): mock melaporkan `conn.ID` yang dilayani, assertion mengikuti ID itu; verifikasi `go test -count=100 -race -run '^TestRefresher_Tick_ContextCancellation$' ./internal/security/oauth/` hijau. Flake (2) `TestKeyRing_ThroughputExceeds10MillionOps` tetap terbuka sebagai risiko lingkungan |
