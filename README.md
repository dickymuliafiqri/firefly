...
- **Grok CLI / Grok Build Adapter (`internal/adapter/grok`):** Routes to the xAI Grok CLI inference API (`cli-chat-proxy.grok.com`, OpenAI Responses API) with an xAI OAuth bearer token, translating OpenAI Chat Completions to/from the Responses wire format (including reasoning/thinking output). Credentials are either pooled from harvested accounts across the key ring or onboarded natively through the OAuth device-code flow (`grok-cli` provider, connect from the dashboard) with automatic proactive token refresh. Recognizes the `grok-build` and `grok-4.5`/`grok-4.6`/`grok-4.7` model families plus `-low`/`-medium`/`-high` effort variants, and its upstream host is pinned rather than operator-editable.
