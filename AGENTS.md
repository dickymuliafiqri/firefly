...
| `openai` | `internal/adapter/openai/` | OpenAI wire-compatible adapter, SSE streaming relay engine (`RelaySSE`), stream idle watchdog, transparent gzip/deflate response decompression (`decodeResponseBody`), and OpenAI error formatting (`WriteError`). |
