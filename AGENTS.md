...
| `opencode` | `internal/opencode/` | OpenCode adapter (`opencode` & `opencode-go` protocols) for OpenCode Zen gateways (`https://opencode.ai/zen/go/v1` and `https://opencode.ai/zen/v1`). Supports both keyless OpenCode Free tier (`Bearer public`, `desktop` client fingerprint, global project ID) and OpenCode Go subscription API keys. Dispatches dual-route traffic (`/responses` for `muse-*`, `grok-*`, `gpt-5.6-luna` with Chat $\rightarrow$ Responses payload translation and SSE stream relay; `/chat/completions` for standard models with tool schema sanitization and OpenAI SSE relay). Enforces session isolation via canonical `x-opencode-session: ses_<descending_id>` conforming to upstream free-tier regex validation. |
